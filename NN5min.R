...
############################ Neural Network########
NNdata_5min <- temp_5min_cal[1:17328,a]

N = nrow(NNdata_5min)
p = ncol(NNdata_5min)

X = dummy.data.frame(NNdata_5min[, -p])
Y = NNdata_5min[, p]
data = cbind(X, Y)
set.seed(1690)
Ind = sample(N, 13000, replace = FALSE) 
p = ncol(data)
...
track = model %>% fit(X_train, Y_train, epochs = 20, batch_size = 25)
plot(track)
