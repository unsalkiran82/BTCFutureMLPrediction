...
############################ Neural Network########
NNdata_1min <- temp_1min_cal[1:86636,a]

N = nrow(NNdata_1min)
p = ncol(NNdata_1min)

X = dummy.data.frame(NNdata_1min[, -p])
Y = NNdata_1min[, p]
data = cbind(X, Y)
set.seed(1690)
Ind = sample(N, 65000, replace = FALSE) 
p = ncol(data)
...
track = model %>% fit(X_train, Y_train, epochs = 20, batch_size = 20)
plot(track)
